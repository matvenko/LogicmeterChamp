...
class champ {
    ...
    function champ_child_info($child_id, $type = ""){
    	...
    	$child_info = $query->select_ar_sql("championship_children", "*", "`date` = '".$this->config['start_date']."' AND child_id = ".(int)$child_id.$where);
    ...
    function get_random_test_ids(){
    	...
    	if((int)$_SESSION['champ']['tests'][$current_skill_id] == 0){
    		$test_id = $query_l->select_ar_sql("math_tests", "id", "del = 0 AND algorithm_id != 0 AND skill_id = ".(int)$current_skill_id." ORDER BY RAND() LIMIT 0, 1");
    ...
    function insert_user_test($grade_id, $skill_id, $test_id, $point_info, $user_answers){
    	global $query, $user_class;
    	if((int)$user_class->current_child_id !== 0){
    		...
    		foreach($user_answers as $answer_n => $answer){
    			...
    			$fields_answer['answer'] = substr(strip_tags($answer), 0, 20);
    ...
    function set_finished_test_status(){
    	...
    	if((int)$stage_info['finished'] == 0 && ((int)$stage_info['tests_amount'] >= (int)$this->config['tests_amount'] || 
    		...
    		$query->update_sql("championship_children_stage_points", array("finished" => 1, "share_code" => $share_code), "child_id = ".(int)$user_class->current_child_id." AND champ_date = '".$this->config['start_date']."'");
