<?php
//if(!in_array($user_class->current_user_id, array(1, 1541, 1538, 1542, 3456))){
...
if(is_post('answer_test')){
	...
	$stage_info = $query->select_ar_sql("championship_children_stage_points", "*", "child_id = ".(int)$user_class->current_child_id." AND champ_date = '".$champ->config['start_date']."'");
    ...
    //***** check if test exist
    if($query->record_exist("championship_children_tests", "child_id = ".(int)$user_class->current_child_id." AND skill_id = ".(int)$test_info['skill_id']) == true){
    	array_shift($_SESSION['champ']['skills']);
    	echo request_callback("ok", "", "reload", array());
        exit;
    }
    ...
    $champ->insert_user_test($test_info['grade_id'], $test_info['skill_id'], $test_id, $point_info, $user_answers);
