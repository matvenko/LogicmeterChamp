...
class eqEOS {
	...
	protected $ST2 = array('+', '-');
	//Allowed functions
	protected $FNC = array('sin', 'cos', 'tan', 'csc', 'sec', 'cot', 'abs', 'log', 'log10', 'sqrt', 'ceil', 'floor', 'round', 'gcd');
    /**#@-*/
	/**
	 * Construct method
	 *
		...
		while((preg_match("/(". implode("|", $this->FNC) . ")\(([^\)\(]*(\([^\)]*\)[^\(\)]*)*[^\)\(]*)\)/", $infix, $match)) != 0) {
			...
			switch($match[1]) {
                ...
                case "floor":
                    ...
                    break;
                case "round":
                    $pars = explode(",", $match[2]);
                    if(count($pars) == 2){
                        $ans = round($pars[0], $pars[1]);
                        
                    }else{
                        $ans = "parameters error";
                    }
                    break;
                case "gcd":
                    $pars = explode(",", $match[2]);
                    if(count($pars) == 2){
                        $gcd = gmp_gcd($pars[0], $pars[1]);
                        $ans = gmp_strval($gcd);
                    }else{
                        $ans = "parameters error";
                    }
